...
const rules: Rule[] = [
  ...
  // handlers injected by the rule, the second one clearing the values set by the first one
  [/^cleared$/, () => ({ name: 'cleared', [symbols.variants]: [
    { parent: ['@media cleared', 5], layer: 'custom', sort: 2 },
    { handle: (input, next) => next({ ...input, parentOrder: undefined, layer: undefined, sort: undefined }) },
  ] as VariantHandler[] })],
  // a position provided by the rule, reversing the alphabetical order (`ms-2` before `ms-1`)
  [/^ms-(\d+)$/, ([, n]) => ({ name: `ms-${n}`, [symbols.sort]: -Number(n) })],
...
const variants: Variant[] = [
  ...
  // a variant resetting the position, like an arbitrary `group-[...]` or a pseudo element
  variantMatcher('su', input => ({ selector: `${input.selector}:su`, sort: undefined })),
...
describe('variantApplyOrder', () => {
  ...
  it('lets the leftmost variant decide the position of the rule and of its block in both orders', async () => {
    for (const config of [{}, { variantApplyOrder: 'left-to-right' as const }]) {
      ...
      // a leftmost variant resetting the `sort` hands the position back to the rule
      const reset = await generate('su:sa:ms-1 su:sa:ms-2', config)
      expect(reset.indexOf('.su\\:sa\\:ms-2')).toBeLessThan(reset.indexOf('.su\\:sa\\:ms-1'))
      // also over a `sort` injected by the rule (2): `su:sorted` falls back to none, before `sb:sorted` (1)
      const injected = await generate('sb:sorted su:sorted', config)
      expect(injected.indexOf('.su\\:sorted')).toBeLessThan(injected.indexOf('.sb\\:sorted'))
    }
  })

  it('lets a handler injected by the rule clear the values set by a previous one in both orders', async () => {
    for (const config of [{}, { variantApplyOrder: 'left-to-right' as const }]) {
      const uno = await createGenerator({ presets: [], rules, variants, ...config })
      const result = await uno.generate('cleared sb:cleared pa:foo', { preflights: false })
      // the layer falls back to the default one
      expect(result.getLayer()).toContain('.cleared')
      expect(result.layers).not.toContain('custom')
      // the block of the rule has no order left (0), before `@media a` (3)
      expect(result.css.indexOf('@media cleared')).toBeLessThan(result.css.indexOf('@media a'))
      // the rule has no position left (0), before `sb:cleared` (1)
      expect(result.css.indexOf('.cleared')).toBeLessThan(result.css.indexOf('.sb\\:cleared'))
    }
  })
