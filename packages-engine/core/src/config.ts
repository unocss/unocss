...
export async function resolveConfig<Theme extends object = object>(
  ...
  function getMerged<T extends 'rules' | 'blocklist' | 'variants' | 'extractors' | 'shortcuts' | 'preflights' | 'preprocess' | 'postprocess' | 'extendTheme' | 'safelist' | 'separators' | 'content' | 'transformers' | 'loaders'>(key: T): ToArray<Required<UserConfig<Theme>>[T]> {
  ...
  const resolved: ResolvedConfig<Theme> = {
    ...
    loaders: uniqueBy(getMerged('loaders'), (a, b) => a.name === b.name),
  }
