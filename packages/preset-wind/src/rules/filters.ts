...
const composePreflightKeys = [...filterBaseKeys, ...backdropFilterBaseKeys]
...
export const filters: Rule<Theme>[] = [
  // filters
  [/^(?:(backdrop-)|filter-)?blur(?:-(.+))?$/, toFilter('blur', (s, theme) => theme.blur?.[s || 'DEFAULT'] || h.bracket.cssvar.px(s)), { preflightKeys: composePreflightKeys, autocomplete: ['(backdrop|filter)-blur-$blur', 'blur-$blur', 'filter-blur'] }],
  [/^(?:(backdrop-)|filter-)?brightness-(.+)$/, toFilter('brightness', s => h.bracket.cssvar.percent(s)), { preflightKeys: composePreflightKeys, autocomplete: ['(backdrop|filter)-brightness-<percent>', 'brightness-<percent>'] }],
  [/^(?:(backdrop-)|filter-)?contrast-(.+)$/, toFilter('contrast', s => h.bracket.cssvar.percent(s)), { preflightKeys: composePreflightKeys, autocomplete: ['(backdrop|filter)-contrast-<percent>', 'contrast-<percent>'] }],
  ...
  [/^(?:(backdrop-)|filter-)?grayscale(?:-(.+))?$/, toFilter('grayscale', percentWithDefault), { preflightKeys: composePreflightKeys, autocomplete: ['(backdrop|filter)-grayscale', '(backdrop|filter)-grayscale-<percent>', 'grayscale-<percent>'] }],
  [/^(?:(backdrop-)|filter-)?hue-rotate-(.+)$/, toFilter('hue-rotate', s => h.bracket.cssvar.degree(s)), { preflightKeys: composePreflightKeys }],
  [/^(?:(backdrop-)|filter-)?invert(?:-(.+))?$/, toFilter('invert', percentWithDefault), { preflightKeys: composePreflightKeys, autocomplete: ['(backdrop|filter)-invert', '(backdrop|filter)-invert-<percent>', 'invert-<percent>'] }],
  // opacity only on backdrop-filter
  [/^(backdrop-)op(?:acity)?-(.+)$/, toFilter('opacity', s => h.bracket.cssvar.percent(s)), { preflightKeys: composePreflightKeys, autocomplete: ['backdrop-(op|opacity)', 'backdrop-(op|opacity)-<percent>'] }],
  [/^(?:(backdrop-)|filter-)?saturate-(.+)$/, toFilter('saturate', s => h.bracket.cssvar.percent(s)), { preflightKeys: composePreflightKeys, autocomplete: ['(backdrop|filter)-saturate', '(backdrop|filter)-saturate-<percent>', 'saturate-<percent>'] }],
  [/^(?:(backdrop-)|filter-)?sepia(?:-(.+))?$/, toFilter('sepia', percentWithDefault), { preflightKeys: composePreflightKeys, autocomplete: ['(backdrop|filter)-sepia', '(backdrop|filter)-sepia-<percent>', 'sepia-<percent>'] }],

  // base
  ['filter', { filter: filterProperty }, { preflightKeys: filterBaseKeys }],
  ['backdrop-filter', {
    '-webkit-backdrop-filter': backdropFilterProperty,
    'backdrop-filter': backdropFilterProperty,
