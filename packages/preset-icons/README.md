...
For example, `vscode-icons:file-type-light-db`, an icon with colors that will be rendered as a mask image and bypass it's colors. Use `vscode-icons:file-type-light-db?bg` to render it as a background image.
