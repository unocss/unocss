...
You must add `Unocss` plugin before `@vitejs/plugin-react`.
...
Since `Web Components` uses `Shadow DOM`, there is no way to style content directly from a global stylesheet (you can use `custom css vars` since those will penetrate the `Shadow DOM`) and so you only need to change the mode of the `UnoCss` plugin to `shadow-dom`.

With the `shadow-dom` mode, the `Unocss` plugin will scope all css styles declared on each individual web component to its own stylesheet, and so you only need to include `@unocss-placeholder` magic placeholder to inject generated css into the corresponding style css block.
...
You can use `::part` since `UnoCss` supports it via `shortcuts` and using `part-[<part-name>]:<shortcut>` rules.

`UnoCss` plugin uses `nth-of-type` to avoid collisions with multiple parts in the same web component and for the same parts on distinct web components, you don't need to worry about it, the plugin will take care for you.
